...
interface SyncDao {
    ...
    @Query(
        ...
        + (SELECT COUNT(*) FROM routine_workouts WHERE syncState = 'PENDING')
        + (SELECT COUNT(*) FROM workout_exercises WHERE syncState = 'PENDING')
        + (SELECT COUNT(*) FROM workout_sets WHERE syncState = 'PENDING')
        + (SELECT COUNT(*) FROM exercises WHERE syncState = 'PENDING')
        AS pendingCount,
        ...
        + (SELECT COUNT(*) FROM routine WHERE syncState = 'FAILED')
        + (SELECT COUNT(*) FROM workout WHERE syncState = 'FAILED')
        + (SELECT COUNT(*) FROM routine_workouts WHERE syncState = 'FAILED')
        + (SELECT COUNT(*) FROM workout_exercises WHERE syncState = 'FAILED')
        + (SELECT COUNT(*) FROM workout_sets WHERE syncState = 'FAILED')
        + (SELECT COUNT(*) FROM exercises WHERE syncState = 'FAILED')
        AS failedCount,
        ...
        + (SELECT COUNT(*) FROM routine WHERE syncState = 'CONFLICT')
        + (SELECT COUNT(*) FROM workout WHERE syncState = 'CONFLICT')
        + (SELECT COUNT(*) FROM routine_workouts WHERE syncState = 'CONFLICT')
        AS conflictCount
        """
    )
